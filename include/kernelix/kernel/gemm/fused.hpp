...
struct SiLUActivation {
    ...
    static __m256 apply_avx(__m256 x) {
        ...
        // Fast exp using Schraudolph's method
...
// =============================================================================
// Micro-Kernel with Compile-Time Activation (No Runtime Branch)
// =============================================================================

/// Micro-kernel that ALWAYS applies activation (for final K tile)
template<std::size_t MR, std::size_t NR, typename T, typename Activation>
struct ActivatedMicroKernel {
    static void run(const T* A, const T* B, T* C, std::size_t K, std::size_t ldc);
};

// Scalar implementation
template<std::size_t MR, std::size_t NR, typename Activation>
struct ActivatedMicroKernel<MR, NR, float, Activation> {
    static void run(const float* A, const float* B, float* C, std::size_t K, std::size_t ldc) {
        float acc[MR][NR] = {};
        ...
        // ALWAYS apply activation (no branch)
        for (std::size_t i = 0; i < MR; ++i) {
            for (std::size_t j = 0; j < NR; ++j) {
...
// =============================================================================
// AVX2 Activated Micro-Kernel (6x16) - No Runtime Branch
// =============================================================================

#if defined(KERNELIX_AVX2)

template<typename Activation>
struct ActivatedMicroKernel<6, 16, float, Activation> {
    static void run(const float* A, const float* B, float* C, std::size_t K, std::size_t ldc) {
        ...
        // ALWAYS apply activation - no runtime branch!
...
// =============================================================================
// Macro-Kernels: Separate paths for regular and activated
// =============================================================================

/// Regular macro-kernel (no activation)
template<typename T, std::size_t MR, std::size_t NR>
void gemm_macro_kernel_regular(
    const T* packed_a, const T* packed_b, T* C,
    std::size_t M, std::size_t N, std::size_t K, std::size_t ldc
) {
    ...
    for (std::size_t jp = 0; jp < n_panels; ++jp) {
        ...
        for (std::size_t ip = 0; ip < m_panels; ++ip) {
            ...
            if (curr_mr == MR && curr_nr == NR) {
                GemmMicroKernel<MR, NR, T>::run(a_panel, b_panel, C + i * ldc + j, K, ldc);
            } else {
                // Edge case
...
/// Activated macro-kernel (applies activation, no runtime branch)
template<typename T, std::size_t MR, std::size_t NR, typename Activation>
void gemm_macro_kernel_activated(
    const T* packed_a, const T* packed_b, T* C,
    std::size_t M, std::size_t N, std::size_t K, std::size_t ldc
) {
    const std::size_t m_panels = (M + MR - 1) / MR;
    ...
    for (std::size_t jp = 0; jp < n_panels; ++jp) {
        ...
        for (std::size_t ip = 0; ip < m_panels; ++ip) {
            ...
            if (curr_mr == MR && curr_nr == NR) {
                ActivatedMicroKernel<MR, NR, T, Activation>::run(
                    a_panel, b_panel, C + i * ldc + j, K, ldc);
            } else {
                // Edge case with activation
                for (std::size_t ii = 0; ii < curr_mr; ++ii) {
                    for (std::size_t jj = 0; jj < curr_nr; ++jj) {
                        T sum = C[(i + ii) * ldc + (j + jj)];
                        for (std::size_t k = 0; k < K; ++k) {
                        ...
                        C[(i + ii) * ldc + (j + jj)] = Activation::apply(sum);
                    }
                }
            }
        }
...
// =============================================================================
// Main Fused GEMM Implementation (Compile-Time Dispatch)
// =============================================================================

/// Fused GEMM + Activation: C = activation(A @ B)
/// Uses separate code paths for regular/activated tiles (no runtime branch in hot path)
        ...
        for (std::size_t j0 = 0; j0 < N; j0 += Cfg::NC) {
            ...
            parallel::parallel_for(
                ...
                [&](std::size_t m_tile) {
                    ...
                    // COMPILE-TIME dispatch: different code paths, no branch in micro-kernel
                    if (is_final_k) {
                        gemm_macro_kernel_activated<T, Cfg::MR, Cfg::NR, Activation>(
                            packed_a.get(), packed_b.get(),
                            C + i0 * ldc + j0, mb, nb, kb, ldc);
                    } else {
                        gemm_macro_kernel_regular<T, Cfg::MR, Cfg::NR>(
                            packed_a.get(), packed_b.get(),
                            C + i0 * ldc + j0, mb, nb, kb, ldc);
                    }
                }
